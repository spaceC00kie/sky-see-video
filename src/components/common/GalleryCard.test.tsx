...
describe('GalleryCard', () => {
  ...
  it('sets lazy loading when lazy prop is true', () => {
    const { getByRole } = renderWithRouter(<GalleryCard {...mockProps} lazy={true} />)
    const image = getByRole('img')
    expect(image).toHaveAttribute('loading', 'lazy')
  })

  it('sets eager loading when lazy prop is false', () => {
    const { getByRole } = renderWithRouter(<GalleryCard {...mockProps} lazy={false} />)
    const image = getByRole('img')
  ...
  it('has proper CSS classes', () => {
    ...
    expect(cardContainer).toHaveClass('flex', 'max-w-xl', 'flex-col', 'items-start', 'gap-2', 'text-lg', 'w-full')
  })
