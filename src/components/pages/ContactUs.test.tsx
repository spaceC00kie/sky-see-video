...
// Also mock it on global in case some code uses global.fetch
...
describe("ContactUs", () => {
  beforeEach(() => {
    ...
    // Reset mock to ensure it returns a successful response
  ...
  // Remove the afterEach check since it conflicts with our safety test
  // The mocking at the top level is sufficient to prevent real API calls
  ...
  it("has correct social media link URLs", () => {
    ...
    const instagramLink = container.querySelector(
      'a[href="https://www.instagram.com/skyseevideo/"]',
    )
    const facebookLink = container.querySelector(
      'a[href="https://www.facebook.com/SkySeeVideo/"]',
    )
  ...
  it("renders service checkboxes", () => {
    ...
    expect(
      getByText(/Production – Aerial & On-The-Ground Videography/),
    ).toBeInTheDocument()
  ...
  it("submits form with valid data", async () => {
    ...
    fireEvent.change(getByLabelText("Description of Project"), {
  ...
  it("shows success message on successful submission", async () => {
    ...
    expect(mockSwal.fire).toHaveBeenNthCalledWith(
      2,
  ...
  it("displays marketing copy", () => {
    const { getByText } = renderWithProviders(<ContactUs />)
    expect(
      getByText("Make the most of your time & budget!"),
    ).toBeInTheDocument()
    expect(
      getByText(/It can be overwhelming to start a new video project/),
    ).toBeInTheDocument()
  })
