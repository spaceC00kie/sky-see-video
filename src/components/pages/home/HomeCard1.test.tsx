...
describe('HomeCard1', () => {
  ...
  it('has correct container structure', () => {
    ...
    expect(container.firstChild).toHaveClass(
      'sm:w-56', 'flex', 'flex-col', 'items-center', 'gap-3', 'justify-between'
    )
  })
  ...
  it('icon container has correct styling', () => {
    const { container } = render(<HomeCard1 {...mockProps} />)
    const iconContainer = container.querySelector('.grid.place-content-center.pb-2')
    expect(iconContainer).toBeInTheDocument()
  })

  it('icon background has correct styling', () => {
    const { container } = render(<HomeCard1 {...mockProps} />)
    const iconBackground = container.querySelector('.grid.h-24.w-28.place-content-center.rounded-full.bg-blue-700')
  ...
  it('has hover effect styling', () => {
    ...
    const shadowElement = container.querySelector('.pointer-events-none.absolute.inset-0.-z-10')
    expect(shadowElement).toBeInTheDocument()
  })

  it('shadow element has correct styling', () => {
    const { container } = render(<HomeCard1 {...mockProps} />)
    const shadowElement = container.querySelector('.translate-x-0\\.5.translate-y-0\\.5.rounded-full.bg-black\\/40.blur-sm')
    expect(shadowElement).toBeInTheDocument()
  })
  ...
  it('handles long title text', () => {
    const longTitleProps = {
      ...mockProps,
      title: 'This is a Very Long Service Title That Should Still Render Properly',
    }
  ...
  it('handles long description text', () => {
    const longDescriptionProps = {
      ...mockProps,
      description: 'This is a very long description that should still render correctly and maintain proper formatting within the home card component structure and layout.',
    }
  ...
  it('renders different icon types correctly', () => {
    const iconProps = {
      ...mockProps,
      image: <div data-testid="different-icon">Different Icon</div> as ReactNode,
    }
