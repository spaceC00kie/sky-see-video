...
const renderWithProviders = (component: React.ReactElement, visibleSlides = 3) => {
  return render(
    <BrowserRouter>
      <CarouselProvider
...
describe('FadeSlider', () => {
  ...
  it('renders images with correct attributes', () => {
    ...
    expect(firstImage).toHaveClass('absolute', 'inset-0', 'h-full', 'w-full', 'object-cover')
  })
  ...
  it('does not render gradient overlays when visibleSlides = 1', () => {
    const { container } = renderWithProviders(<FadeSlider visibleSlides={1} />, 1)
  ...
  it('has correct link structure', () => {
    ...
    expect(links[0]).toHaveClass('flex', 'h-full', 'flex-col', 'overflow-hidden')
  })
  ...
  it('has correct heading styles', () => {
    ...
    expect(heading).toHaveClass('mb-1', 'text-3xl', 'font-semibold', 'sm:text-lg')
  })
  ...
  it('has correct description styles', () => {
    ...
    expect(description).toHaveClass('line-clamp-2', 'text-2xl', 'text-gray-700', 'sm:text-sm')
  })
