...
jest.mock("pure-react-carousel", () => ({
  CarouselProvider: ({ children }: { children: React.ReactNode }) => (
    <div data-testid="carousel-provider">{children}</div>
  ),
  ButtonBack: ({ children }: { children: React.ReactNode }) => (
    <button data-testid="button-back">{children}</button>
  ),
  ButtonNext: ({ children }: { children: React.ReactNode }) => (
    <button data-testid="button-next">{children}</button>
  ),
}))
...
jest.mock("./FadeSlider", () => ({
  FadeSlider: ({ visibleSlides }: { visibleSlides: number }) => (
    <div data-testid="fade-slider" data-visible-slides={visibleSlides}>
      Fade Slider
    </div>
  ),
}))
...
const renderWithProviders = (
  ...
  initialPath = "/project/current-project",
) => {
...
describe("ProjectPage", () => {
  ...
  it("renders carousel with correct visible slides for different screen sizes", () => {
    ...
    expect(getByTestId("fade-slider")).toHaveAttribute(
      "data-visible-slides",
      "3",
    )
  })
  ...
  it("video container has correct aspect ratio", () => {
    ...
    const videoContainer = container.querySelector(
      ".relative.mx-auto.mb-6.aspect-video.w-full.max-w-\\[73em\\]",
    )
  ...
  it("handles long title and description gracefully", () => {
    const longProps = {
      title:
        "This is a Very Long Project Title That Should Still Display Correctly",
      description:
        "This is a very long description that goes into great detail about the project, its goals, the production process, and all the various aspects that make it unique and interesting.",
